...
A number of the classes that were generated by JAXB are extended to provide some important extra functionality. These classes have a name ending on "Ex", like `RefTypeEx` and `XhtmlTextEx`. 
During traversal of the information model an object of for instance the class `RefType` or `XhtmlText` can always be cast to `RefTypeEx` and `XhtmlTextEx` to access the extra functionality.
